/**
 * useNavigator — CSS transform-based stack navigation.
 *
 ...
 * for predictable animations that don't conflict with Telegram's
 * native UI mutations (MainButton/BackButton viewport resizing).
 *
 ...
 * the transition completes. This prevents cleanup side effects
 * (MainButton.hide, BackButton.hide) from firing mid-animation.
 */
...
export function useNavigator(): Navigator {
  ...
  const replace = useCallback((screen: Screen) => {
    // Instant data swap — no animation. Used when a build completes
    // while the user is viewing it (active → recent transition).
    setCurrent(screen);
  }, []);
