"""Stack Manager API client.

Provides a typed async interface to the stack-manager builds API.
...
class BuildResult:
    """A completed build returned by the stack-manager API."""
...
class SMClientError(Exception):
    """Raised when a stack-manager API call fails.
...
class SMClient:
    """Async HTTP client for the stack-manager builds API."""
    ...
    async def trigger_build(
        ...
        """Trigger a build via stack-manager.

        Returns ``{request_id, status}`` on success.

        Raises ``SMClientError`` on failure.
        """
        ...
        try:
            ...
            logger.exception("Failed to reach stack-manager for build trigger")
            raise SMClientError(
        ...
        # SM returns user-friendly errors in `detail`
    ...
    async def cancel_build(self, request_id: str) -> dict[str, str]:
        """Cancel a pending build via stack-manager."""
    ...
    async def get_recent_builds(
        ...
        """Fetch recent completed builds from stack-manager."""
    ...
    async def get_build_status(self) -> dict[str, Any]:
        """Fetch build orchestrator status from stack-manager."""
