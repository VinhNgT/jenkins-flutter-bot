...
from service_hub.jenkins_pipeline import generate_jenkinsfile
...
async def test_manager_get_jenkinsfile_explicit_params():
    """Verify ServiceHubManager.get_jenkinsfile works with explicit params and has no warnings."""
    config = ServiceHubBootstrap(
        agent_control_url=None,
        file_manager_url=None,
        build_manager_url=None,
    )
    manager = ServiceHubManager(config=config)
