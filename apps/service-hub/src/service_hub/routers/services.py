...
_MANAGED_SERVICES = {"agent-control", "file-manager", "build-manager"}
...
async def get_service_status(manager: ManagerDep) -> dict[str, Any]:
    ...
    scopes = ["agent-control", "file-manager", "build-manager"]
...
async def get_service_logs(manager: ManagerDep, service: str) -> dict[str, Any]:
...
@router.post("/agent-control/vpn/upload")
async def proxy_vpn_upload(
    ...
    return await manager.services.upload_vpn_file(
        content, file.filename or "client.ovpn"
    )


@router.get("/agent-control/vpn/status")
...
@router.delete("/agent-control/vpn/upload")
...
@router.post("/agent-control/vpn/connect")
...
@router.post("/agent-control/vpn/disconnect")
