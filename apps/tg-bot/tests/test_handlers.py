...
from tg_bot.bot.handlers import start_handler, help_handler, status_handler
...
class TestCommandHandlers:
    ...
    async def test_status_handler_ready(self, ctx, bot) -> None:
        ...
        ctx.build_client.get_pending_builds = AsyncMock(return_value={})
        ...
        assert "Service: <code>tg-bot v" in text
    ...
    async def test_status_handler_building(self, ctx, bot) -> None:
        ...
        ctx.build_client.get_pending_builds = AsyncMock(
            return_value={
                "req-111": {
                    "branch": "develop",
