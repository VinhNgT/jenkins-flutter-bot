"""ServiceHubManager — centralized configuration proxy.

Service-hub has zero domain schemas of its own. All config I/O is
...
from .config import ServiceHubBootstrap
from .env_io import (
...
class ServiceHubManager:
    ...
    def __init__(
        self,
        *,
        config: ServiceHubBootstrap | None = None,
        service_client: ServiceClient | None = None,
        fm_client: httpx.AsyncClient | None = None,
    ) -> None:
        resolved = config or ServiceHubBootstrap.load()
    ...
    async def start(self) -> None:
        """No-op — service-hub has no daemon to start."""
    ...
    async def restart(self, config: ServiceHubBootstrap | None = None) -> None:
        ...
        logger.info("ServiceHubManager restarted")

    def status(self) -> dict[str, Any]:
        """Return standardized status — service-hub is always running."""
        return {
            "configured": True,
            "running": True,
            "last_error": None,
    ...
    async def export_env(self) -> dict[str, Any]:
        ...
        compose_env_str, warnings = generate_compose_env(
            agent_config=configs.get("agent-control", {}),
            agent_schema=schemas.get("agent-control"),
            file_manager_config=configs.get("file-manager", {}),
            file_manager_schema=schemas.get("file-manager"),
            build_manager_config=configs.get("build-manager", {}),
            build_manager_schema=schemas.get("build-manager"),
        )
    ...
    async def import_tarball(self, raw: bytes) -> dict[str, Any]:
        """Import a config tarball and apply changes.
        ...
        (
            agent_schema,
            file_manager_schema,
            build_manager_schema,
        ) = await asyncio.gather(
            self.services.schema("agent-control"),
            self.services.schema("file-manager"),
            self.services.schema("build-manager"),
        )
        ...
        parsed = import_tarball(
            ...
            build_manager_schema=build_manager_schema,
        )
    ...
    async def get_jenkinsfile(
        ...
        if not repo_url or not credentials_id:
            orch_config = await self.services.get_config("build-manager")
