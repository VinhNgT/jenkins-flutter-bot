"""Tg-bot — FastAPI app factory and CLI."""
...
def create_app() -> FastAPI:
    ...
    static_admin_dir = Path(__file__).parent / "webapp-admin"
    static_admin_dir.mkdir(parents=True, exist_ok=True)
    admin_index_html = static_admin_dir / "index.html"
    if not admin_index_html.exists():
        admin_index_html.write_text("<!DOCTYPE html><html><body>Admin Dashboard Placeholder</body></html>")
    app.mount(
        "/webapp-admin", StaticFiles(directory=str(static_admin_dir), html=True), name="webapp-admin"
    )
...
def cli() -> None:
    """CLI entry point for the tg-bot service."""
