...
def _build_env_lines(
    ...
    separate section, always commented out and always empty.
    """
...
def generate_compose_env(
    ...
    build_manager_config: dict[str, Any] | None = None,
    build_manager_schema: dict[str, Any] | None = None,
) -> tuple[str, list[str]]:
    ...
    configs = [
        ...
        (build_manager_schema, build_manager_config, "Build Manager"),
    ]
...
def build_export_tarball(
    ...
    with tarfile.open(fileobj=buf, mode="w:gz") as tar:
        ...
        filename_map = {
            "agent-control": "agent.json",
            "file-manager": "storage.json",
            "build-manager": "builds.json",
        }
...
def _build_env_lookup(
    ...
    only write portable config to the JSON files.
    """
    if not schema or "fields" not in schema:
        return {}
...
def _parse_env_content(
    ...
    build_manager_lookup: dict[str, dict[str, Any]] | None = None,
) -> tuple[
    dict[str, Any],
    dict[str, Any],
    dict[str, Any],
    ...
    """Parse env file content and route values to agent/file_manager/builds patches."""
    applied: list[str] = []
    ...
    build_manager_patch: dict[str, Any] = {}

    _file_manager_lookup = file_manager_lookup or {}
    _build_manager_lookup = build_manager_lookup or {}
    ...
    for line_num, raw_line in enumerate(content.splitlines(), start=1):
        ...
        if env_var in agent_lookup:
            ...
            scope = "file-manager"
        elif env_var in _build_manager_lookup:
            field_def = _build_manager_lookup[env_var]
            target_patch = build_manager_patch
            scope = "build-manager"
        else:
...
def import_tarball(
    ...
    build_manager_lookup = _build_env_lookup(build_manager_schema)
        ...
        with tarfile.open(fileobj=io.BytesIO(tarball_bytes), mode="r:gz") as tar:
            for member in tar.getmembers():
                ...
                if basename in (
                    ...
                    scope_map = {
                        "agent.json": "agent-control",
                        "storage.json": "file-manager",
                        "builds.json": "build-manager",
                    }
                ...
                # Process compose.env
                    ...
                    ap, fmp, bu_p, applied, skipped, unrec, errors = (
                        _parse_env_content(
                            content,
                            agent_lookup,
                            file_manager_lookup,
                    ...
                    doc_agent = ConfigDocument(configs["agent-control"])
                    doc_agent.merge(ap)
                    configs["agent-control"] = doc_agent.data

                    doc_fm = ConfigDocument(configs["file-manager"])
                    doc_fm.merge(fmp)
                    configs["file-manager"] = doc_fm.data

                    doc_bu = ConfigDocument(configs["build-manager"])
                    doc_bu.merge(bu_p)
                    configs["build-manager"] = doc_bu.data
