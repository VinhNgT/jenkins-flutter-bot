...
Both `config-ui` and `tg-admin-bot` control services via `ServiceClient` from `stack-manager`:
...
If a service is down, its schema returns `null` and the config-ui frontend shows "Loading..." for that tab.
...
1. **Browser-redirect flow** — used by `config-ui` (web dashboard with popup callback)
...
- **Frontend-only "Connecting" state** — the config-ui popup flow is tracked entirely by the frontend via a `<dialog>` modal. Do NOT add a backend `auth_pending` flag.
...
Both `config-ui` and `tg-admin-bot` support symmetric config transfer via `stack-manager`:
