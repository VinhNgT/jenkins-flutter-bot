"""Smoke tests for service-hub API endpoints."""
...
async def test_version_denormalization(client):
    ...
    with patch("service_hub.routers.version.version") as mock_version:
        # Dev pre-release
...
async def test_services_status(client):
    ...
    for service in ("agent-control", "file-manager", "build-manager"):
