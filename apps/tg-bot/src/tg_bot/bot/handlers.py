...
def _bot_version() -> str:
    """Return the installed tg-bot package version, or 'unknown'."""
    try:
        v = _pkg_version("tg-bot")
        import re
...
async def status_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Fetch pending builds from build-manager
    pending = await ctx.build_client.get_pending_builds()
    ...
    lines = [
        ...
        "",
        f"Service: <code>tg-bot v{version}</code>",
        "Status: 🟢 Online",
        f"Active builds: {len(pending)}",
    ]
    ...
    if pending:
        ...
        for req_id, b in pending.items():
            branch = b.get("branch", "unknown")
            triggered_at = b.get("triggered_at", 0.0)
            short_req = req_id[:8]
            lines.append(
                f"  • {_escape(branch)} (req: <code>{_escape(short_req)}</code>, started {ctx.format_elapsed(triggered_at)})"
            )
