...
from config_core import ServiceSettings, BootstrapSettings

_DEFAULT_CONFIG_PATH = Path("/app/data/bot.json")


class BotBootstrap(BootstrapSettings):
    """Resolved tg-bot bootstrap configuration."""

    telegram_bot_token: str = Field(
        ...,
        description="Telegram bot token from @BotFather",
    )
    service_hub_url: str | None = Field(
        None,
        description="Internal URL of the service-hub coordinator",
    )
    admin_telegram_user_ids: list[int] = Field(
        default_factory=list,
        description="List of Telegram user IDs allowed to access the admin dashboard",
    )
    ...
    def validate_preview_credentials(self) -> BotBootstrap:
...
class BotSettings(ServiceSettings):
    ...
    bot_service_url: str = Field(
        ...
        json_schema_extra={
            "group": "Advanced",
            "order": 1,
            "help_html": "Internal URL where this bot receives webhooks (e.g., <code>http://tg-bot:9090</code>). Normally provided by the deployment environment.",
